...
const EntityMapScreen = ({ route, navigation }) => {
  ...
  return (
    <Container>
        ...
        <View style={styles.containerMap}>
          <MapView style={styles.map} region={{ latitude: parseFloat(currentLatitude), longitude: parseFloat(currentLongitude), latitudeDelta: 0.5, longitudeDelta: 0.5, }}>
            ...
            {route.params.locations.map(marker => (
              <Marker key={marker.id} coordinate={marker.coordinates} title={marker.title}/>
            ))}
          </MapView>
        </View>
      </Content>
      <Footer>
        <FooterTab >
          <Button vertical active style={{ backgroundColor: color, borderRadius: 0}} onPress={() => navigation.navigate('Home')}>
            <Icon name="home" />
            <Text>{t('Home')}</Text>
          </Button>
          <Button vertical active style={{ backgroundColor: color, borderRadius: 0}}  onPress={() => navigation.navigate('Shops')}>
...
export default EntityMapScreen;
